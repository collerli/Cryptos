...
contract Token {
    ...
    event Mint(address indexed to, uint amount);  // Evento de mint
    ...
    // Função de mint para enviar tokens para uma carteira específica
    function mint(address to, uint amount) public onlyOwner {
        ...
        balances[to] += amountInWei;
        emit Transfer(address(0), to, amountInWei);
        emit Mint(to, amountInWei);  // Emitindo o evento de mint
    }
}
